...
import com.wichat.android.identity.SecureIdentityStateManager
import com.wichat.android.wifi.PeerFingerprintManager
import com.wichat.android.noise.southernstorm.protocol.Noise
...
class NoiseEncryptionService(private val context: Context) {
     ...
     * This preserves peer relationships for automatic session re-establishment
     */
    fun saveSessions() {
        Log.d(TAG, "Saving Noise sessions for background persistence")
        sessionManager.saveSessions(context)
        
        // Also save peer fingerprints through the centralized manager
        fingerprintManager.saveToStorage()
    }
    
    /**
     * Restore sessions from persistent storage
     * Re-establishes peer relationships and triggers automatic handshakes as needed
     */
    fun restoreSessions() {
        Log.d(TAG, "Restoring Noise sessions from persistent storage")
        sessionManager.restoreSessions(context)
        
        // Restore peer fingerprints through the centralized manager
        fingerprintManager.restoreFromStorage()
    }
    
    /**
     * Clear all saved session data (for privacy/security)
     */
    fun clearSavedSessions() {
        Log.d(TAG, "Clearing all saved Noise session data")
        sessionManager.clearSavedSessions(context)
        fingerprintManager.clearStorage()
    }
    
    /**
     * Get established peer IDs for background session maintenance
     */
    fun getEstablishedPeerIDs(): Set<String> {
        return sessionManager.sessions.entries
            .filter { it.value.isEstablished() }
            .map { it.key }
            .toSet()
    }
    
    /**
     * Auto-establish sessions with known peers
     * This should be called when the background service starts
     */
    fun autoEstablishKnownPeerSessions(availablePeerIDs: Set<String>) {
        val knownFingerprints = fingerprintManager.getAllPeerFingerprints()
        val knownPeers = knownFingerprints.keys
        
        Log.d(TAG, "Auto-establishing sessions: ${knownPeers.size} known peers, ${availablePeerIDs.size} available")
        
        for (knownPeerID in knownPeers) {
            // If this peer is currently available but we don't have a session
            if (availablePeerIDs.contains(knownPeerID) && !hasEstablishedSession(knownPeerID)) {
                Log.d(TAG, "Auto-establishing session with known peer $knownPeerID")
                
                // Trigger handshake - this will be handled by the normal handshake flow
                onHandshakeRequired?.invoke(knownPeerID)
            }
        }
    }
    ...
    private fun generateKeyPair(): Pair<ByteArray, ByteArray> {
        try {
            val dhState = com.wichat.android.noise.southernstorm.protocol.Noise.createDH("25519")
