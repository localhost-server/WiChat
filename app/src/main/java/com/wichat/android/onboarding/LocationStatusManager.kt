...
class LocationStatusManager(
    ...
    private fun setupLocationSettingsLauncher() {
        locationSettingsLauncher = activity.registerForActivityResult(
            ...
            if (isEnabled) {
                ...
                onLocationDisabled("Location services are required for WiFi scanning on Android. Please enable location services to continue.")
            }
        }
    }
    ...
    fun getStatusMessage(status: LocationStatus): String {
        return when (status) {
            ...
            LocationStatus.DISABLED -> "Location services are disabled. Please enable location services for WiFi scanning."
