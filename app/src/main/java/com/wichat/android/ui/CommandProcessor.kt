...
class CommandProcessor(
    ...
    fun processCommand(command: String, meshService: WifiMeshService, myPeerID: String, onSendMessage: (String, List<String>, String?) -> Unit): Boolean {
    ...
    private fun handleMessageCommand(parts: List<String>, meshService: WifiMeshService) {
        if (parts.size > 1) {
    ...
    private fun handleWhoCommand(meshService: WifiMeshService) {
    ...
    private fun handleBlockCommand(parts: List<String>, meshService: WifiMeshService) {
        if (parts.size > 1) {
            val targetName = parts[1].removePrefix("@")
    ...
    private fun handleUnblockCommand(parts: List<String>, meshService: WifiMeshService) {
        if (parts.size > 1) {
            val targetName = parts[1].removePrefix("@")
    ...
    private fun getPeerIDForNickname(nickname: String, meshService: WifiMeshService): String? {
        return try {
    ...
    private fun getPeerNickname(peerID: String, meshService: WifiMeshService): String {
        return try {
            val method = meshService::class.java.getDeclaredMethod("getPeerNicknames")
            val peerNicknames = method.invoke(meshService) as? Map<String, String>
    ...
    private fun getMyPeerID(meshService: WifiMeshService): String {
        return try {
    ...
    private fun sendPrivateMessageVia(meshService: WifiMeshService, content: String, peerID: String, recipientNickname: String, messageId: String) {
        try {
