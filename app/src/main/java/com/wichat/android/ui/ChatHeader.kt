...
import com.wichat.android.core.ui.utils.singleOrTripleClickable
import com.wichat.android.model.VoiceCallState
import com.wichat.android.ui.voice.CallButton
import com.wichat.android.ui.voice.VoiceCallStateIndicator
...
fun ChatHeaderContent(
    ...
    when {
        selectedPrivatePeer != null -> {
            ...
            // Observe voice call state for this peer
            ...
            PrivateChatHeader(
                ...
                onStartVoiceCall = { viewModel.initiateVoiceCall(selectedPrivatePeer) }
            )
        }
        currentChannel != null -> {
            // Channel header
...
private fun PrivateChatHeader(
    ...
    Box(modifier = Modifier.fillMaxWidth()) {
        ...
        // Right side buttons (voice call and favorite) - positioned on the right
        Row(
            modifier = Modifier.align(Alignment.CenterEnd),
            horizontalArrangement = Arrangement.spacedBy(4.dp),
            verticalAlignment = Alignment.CenterVertically
        ) {
            // Voice call state indicator (if in call)
            if (voiceCallState != VoiceCallState.IDLE) {
                VoiceCallStateIndicator(
                    state = voiceCallState,
                    modifier = Modifier.size(24.dp)
                )
            }
            
            // Voice call button (only show when not in a call with this peer)
            if (voiceCallState == VoiceCallState.IDLE || 
                (currentVoiceCall?.get("peerID") as? String) != peerID) {
                CallButton(
                    onClick = onStartVoiceCall,
                    enabled = voiceCallState == VoiceCallState.IDLE,
