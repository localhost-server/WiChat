...
class PeerFingerprintManager private constructor() {
    ...
    /**
     * Save fingerprint mappings to persistent storage (placeholder)
     * Currently fingerprints are managed in memory only
     */
    fun saveToStorage() {
        Log.d(TAG, "Fingerprint persistence: ${peerIDToFingerprint.size} mappings (in-memory only)")
        // Note: Fingerprints are currently managed in memory only
        // This could be extended to SharedPreferences if longer-term persistence is needed
    }
    
    /**
     * Restore fingerprint mappings from persistent storage (placeholder) 
     * Currently fingerprints are managed in memory only
     */
    fun restoreFromStorage() {
        Log.d(TAG, "Fingerprint restore: no persistent storage configured (in-memory only)")
        // Note: Fingerprints are currently managed in memory only
        // This could be extended to SharedPreferences if longer-term persistence is needed
    }
    
    /**
     * Clear persistent storage (placeholder)
     * Currently fingerprints are managed in memory only
     */
    fun clearStorage() {
        Log.d(TAG, "Fingerprint storage clear: in-memory only")
        // Note: Fingerprints are currently managed in memory only
    }
