...
class SecurityManager(private val encryptionService: EncryptionService, private val myPeerID: String) {
    ...
    suspend fun handleNoiseHandshake(routed: RoutedPacket, step: Int): Boolean {
        ...
        if (encryptionService.hasEstablishedSession(peerID)) {
            if (step == 1) {
                // Step 1 handshake means peer wants to reset/re-establish session
                Log.w(TAG, "Peer $peerID wants to reset session (step 1 handshake) - allowing session reset")
                encryptionService.removePeer(peerID)  // Remove existing session to allow reset
            } else {
    ...
    private fun cleanupOldData() {
        ...
        if (processedMessages.size > MAX_PROCESSED_MESSAGES) {
            ...
            val toRemove = synchronized(processedMessages) {
                processedMessages.take(excess)
            }
        ...
        if (processedKeyExchanges.size > MAX_PROCESSED_KEY_EXCHANGES) {
            ...
            val toRemove = synchronized(processedKeyExchanges) {
                processedKeyExchanges.take(excess)
            }
