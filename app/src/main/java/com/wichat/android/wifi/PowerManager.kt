...
/**
 * Power-aware WiFi management for bitchat
 * Adjusts discovery, connection behavior based on battery state
 */
class PowerManager(private val context: Context) {
    ...
    companion object {
        ...
        // WiFi discovery duty cycle periods (ms)
        private const val DISCOVERY_ON_DURATION_NORMAL = 10000L    // 10 seconds on
        private const val DISCOVERY_OFF_DURATION_NORMAL = 5000L    // 5 seconds off
        private const val DISCOVERY_ON_DURATION_POWER_SAVE = 5000L    // 5 seconds on
        private const val DISCOVERY_OFF_DURATION_POWER_SAVE = 15000L  // 15 seconds off
        private const val DISCOVERY_ON_DURATION_ULTRA_LOW = 2000L      // 2 seconds on
        private const val DISCOVERY_OFF_DURATION_ULTRA_LOW = 30000L   // 30 seconds off
    ...
    /**
     * Get WiFi discovery interval optimized for current power mode
     */
    fun getWifiDiscoveryInterval(): Long {
        return when (currentMode) {
            PowerMode.PERFORMANCE -> 5000L      // 5 seconds
            PowerMode.BALANCED -> 10000L        // 10 seconds
            PowerMode.POWER_SAVER -> 20000L     // 20 seconds
            PowerMode.ULTRA_LOW_POWER -> 60000L // 60 seconds
        }
    }
    
    /**
     * Get WiFi connection timeout optimized for current power mode
     */
    fun getWifiConnectionTimeout(): Int {
        return when (currentMode) {
            PowerMode.PERFORMANCE -> 10000      // 10 seconds
            PowerMode.BALANCED -> 8000          // 8 seconds
            PowerMode.POWER_SAVER -> 5000       // 5 seconds
            PowerMode.ULTRA_LOW_POWER -> 3000   // 3 seconds
        }
    }
    ...
    private fun startDutyCycle() {
        ...
        val (onDuration, offDuration) = when (currentMode) {
            PowerMode.BALANCED -> DISCOVERY_ON_DURATION_NORMAL to DISCOVERY_OFF_DURATION_NORMAL
            PowerMode.POWER_SAVER -> DISCOVERY_ON_DURATION_POWER_SAVE to DISCOVERY_OFF_DURATION_POWER_SAVE
            PowerMode.ULTRA_LOW_POWER -> DISCOVERY_ON_DURATION_ULTRA_LOW to DISCOVERY_OFF_DURATION_ULTRA_LOW
