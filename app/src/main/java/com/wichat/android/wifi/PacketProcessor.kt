            ...
            MessageType.FRAGMENT_END -> handleFragment(routed)
                ...
                if (packetRelayManager.isPacketAddressedToMe(packet)) {
                    when (messageType) {
                        ...
                        MessageType.VOICE_CALL_OFFER -> handleVoiceCallOffer(routed)
                        MessageType.VOICE_CALL_ANSWER -> handleVoiceCallAnswer(routed)
                        MessageType.VOICE_CALL_REJECT -> handleVoiceCallReject(routed)
                        MessageType.VOICE_CALL_HANGUP -> handleVoiceCallHangup(routed)
                        MessageType.VOICE_AUDIO_DATA -> handleVoiceAudioData(routed)
                        else -> {
                            validPacket = false
...
//    }
    ...
    // MARK: - Voice Call Handlers
    
    /**
     * Handle voice call offer
     */
    private fun handleVoiceCallOffer(routed: RoutedPacket) {
        val peerID = routed.peerID ?: "unknown"
        Log.d(TAG, "Processing voice call offer from ${formatPeerForLog(peerID)}")
        delegate?.handleVoiceCallOffer(routed)
    }
    
    /**
     * Handle voice call answer
     */
    private fun handleVoiceCallAnswer(routed: RoutedPacket) {
        val peerID = routed.peerID ?: "unknown"
        Log.d(TAG, "Processing voice call answer from ${formatPeerForLog(peerID)}")
        delegate?.handleVoiceCallAnswer(routed)
    }
    
    /**
     * Handle voice call reject
     */
    private fun handleVoiceCallReject(routed: RoutedPacket) {
        val peerID = routed.peerID ?: "unknown"
        Log.d(TAG, "Processing voice call reject from ${formatPeerForLog(peerID)}")
        delegate?.handleVoiceCallReject(routed)
    }
    
    /**
     * Handle voice call hangup
     */
    private fun handleVoiceCallHangup(routed: RoutedPacket) {
        val peerID = routed.peerID ?: "unknown"
        Log.d(TAG, "Processing voice call hangup from ${formatPeerForLog(peerID)}")
        delegate?.handleVoiceCallHangup(routed)
    }
    
    /**
     * Handle voice audio data
     */
    private fun handleVoiceAudioData(routed: RoutedPacket) {
        val peerID = routed.peerID ?: "unknown"
        Log.d(TAG, "Processing voice audio data from ${formatPeerForLog(peerID)}")
        delegate?.handleVoiceAudioData(routed)
    }
...
//    fun handleDeliveryAck(routed: RoutedPacket)
    ...
    // Voice call handlers
    fun handleVoiceCallOffer(routed: RoutedPacket)
    fun handleVoiceCallAnswer(routed: RoutedPacket)
    fun handleVoiceCallReject(routed: RoutedPacket)
    fun handleVoiceCallHangup(routed: RoutedPacket)
    fun handleVoiceAudioData(routed: RoutedPacket)
}
