...
import com.wichat.android.model.NoiseIdentityAnnouncement
            ...
            if (decryptedData.size > 1) {
                ...
                if (typeMarker == MessageType.READ_RECEIPT.value) {
                    ...
                    if (receipt != null) {
                        ...
                        Log.i(TAG, "✓ Processed read receipt from $peerID for message ${receipt.originalMessageID}")
                    } else {
                        Log.w(TAG, "Failed to decode read receipt from $peerID")
                    }
                    return
                }
            }
