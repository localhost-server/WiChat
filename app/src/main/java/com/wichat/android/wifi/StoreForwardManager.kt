...
class StoreForwardManager {
    ...
    fun shouldCacheForPeer(recipientPeerID: String): Boolean {
        // Check if recipient is offline - cache for ALL offline peers
        val isOffline = !(delegate?.isPeerOnline(recipientPeerID) ?: false)
        
        // Cache messages for all offline peers, not just favorites
        // Favorites get longer retention, but all peers get store-and-forward
        return isOffline
    }
